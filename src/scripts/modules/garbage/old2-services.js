...
if (cards) {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const onMouseEnterHandler = (evt) => {
      ...
      let cardAnimation = gsap.timeline({ paused: true, delay: 0.2 });
    ...
    const init = () => {
      ...
      if (contentWidth > minContentWidth) {
        ...
        cards.forEach((card, index) => {
          ...
          card.style.left =
            cardWidth * index + offsetWidth - offsetWidth / 2 + "px";
        });
      } else {
      ...
      setTimeout(() => {
        services.style.minHeight = `${
          (cards[cards.length - 1].getBoundingClientRect().height / 100) * 90
        }px`;
      }, 0);
    };
